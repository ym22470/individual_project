...
DEFAULT_LOG_FILE = "/home/chongshengwang/naratix/benchmark_cache/gradio_demo_test_chromadb.out"
...
def sanitize_collection_name(name: str) -> str:
    ...
    return cleaned[:63].strip("_") or "video_eval"
...
def create_fresh_collection(client, collection_name: str):
    try:
...
def index_docs_in_chromadb(collection, docs: List[Dict[str, str]], embedding_model):
    embeddings = embedding_model.encode(
        [f"search_document: {doc['text']}" for doc in docs]
    ).tolist()
    ...
    collection.add(
        ...
        metadatas=[
            {"file_name": doc["file_name"], "path": doc["path"]}
            for doc in docs
        ],
...
def evaluate(collection, embedding_model, benchmark_queries: List[Dict[str, str]]):
    cosine_hits = 0
    cosine_hits_at_5 = 0
    cosine_mrr = 0.0
    cosine_latency = 0.0
    ...
    for row in benchmark_queries:
        ...
        query_embedding = embedding_model.encode(f"search_query: {query}").tolist()
        ...
        cosine_top1 = ranked_ids[0]
        cosine_top5 = ranked_ids[:5]
...
def run_model_benchmark(
    ...
    for video_path in selected_videos:
        ...
        print(f"[{model_label}] [{len(docs) + 1}/{len(selected_videos)}] Analyzing {file_name}...")
    ...
    model_chroma_dir = os.path.join(chroma_root, model_label)
    if os.path.exists(model_chroma_dir):
        shutil.rmtree(model_chroma_dir)
    os.makedirs(model_chroma_dir, exist_ok=True)

    chroma_client = chromadb.PersistentClient(path=model_chroma_dir)
    collection = create_fresh_collection(
        chroma_client,
        sanitize_collection_name(f"{model_label}_video_embeddings"),
    )
    index_docs_in_chromadb(collection, docs, embedding_model)

    print(f"\nBenchmark set for {model_label}:")
    for row in benchmark_queries:
        print(f"  - query='{row['query']}' -> target={row['target_id']}")

    metrics = evaluate(collection, embedding_model, benchmark_queries)
...
def print_model_comparison(results: List[Dict]):
    ...
    latency_row = f"{'Cosine latency (ms)':<{label_width}}" + "".join(
        f"{result['metrics']['cosine_avg_latency_ms']:>{value_width}.2f}" for result in results
    )
    print(latency_row)


def main():
    parser = argparse.ArgumentParser(description="Batch test cosine retrieval on local videos using ChromaDB")
    parser.add_argument("--videos-dir", default=DEFAULT_VIDEOS_DIR, help="Directory containing test videos")
    ...
    parser.add_argument(
        ...
        help="Directory for storing temporary ChromaDB benchmark collections",
    )
    parser.add_argument(
        "--log-file",
        default=DEFAULT_LOG_FILE,
        help="Path to a .out file that will receive a copy of the benchmark logs",
    )
    ...
    with open(args.log_file, "w", encoding="utf-8") as log_handle:
